...
function exportCompletionLogCSV() {
    const header = 'Timestamp,Duration (seconds)\n';
    const rows = completionLog.map((entry) => `${entry.timestamp},${entry.duration}`).join('\n');
    return header + rows;
}

function downloadCompletionLogCSV() {
    const csvContent = exportCompletionLogCSV();
    const blob = new Blob([csvContent], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'tempo_completion_log.csv';
